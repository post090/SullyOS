...
export interface SongSheet {
    ...
    theme?: string; // 歌曲主题/主旨（记忆宫殿注入文本用，可选）
    lines: SongLine[];
...
export interface CharMusicProfile {
    ...
    signatureArtists: { name: string; artistId?: number; starred?: boolean; picUrl?: string }[];
    /** 偏爱的影视/音乐剧/游戏 OST 等（独立于纯音乐艺人，填充歌单时按类型加搜索后缀） */
    favoriteSoundtracks?: { title: string; type: 'ost' | 'musical' | 'film' | 'game' | 'anime'; starred?: boolean; coverUrl?: string }[];
    /** 本地歌单列表 */
