...
import { fetchWeatherWithFallback } from './realtimeWorldCore';
...
export interface AgenticToolChar {
    ...
    /** fork 扩展：角色级地区覆盖（双城天气 / 热点平台过滤），上游无此字段 */
    regionConfig?: { city?: string };
}
 ...
 * 用 realtimeWorldCore 的 fetchWeatherWithFallback 直取——浏览器和 amsg worker 共用同一份取数逻辑，
 * 模糊注入和精确查询走同一套源，不会出现"直觉说凉快、一查却在下雹子"的精分现场。
 */
export async function runCheckWeather(
    ...
    try {
        ...
        const [userW, charW] = await Promise.all([
            fetchWeatherWithFallback(userCityRaw || '', realtimeConfig.weatherApiKey),
            sameCity ? Promise.resolve(null) : fetchWeatherWithFallback(charCityRaw, realtimeConfig.weatherApiKey),
        ]);
        if (!userW && !charW) {
