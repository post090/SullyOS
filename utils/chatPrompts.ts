...
重要的不是回应长成什么样子，重要的是——ta 的表达，真的改变了你接下来怎么回。`;
        ...
        // 末尾 recency 第二棒：把「你是谁」拉到整段 prompt 的最后一句。
        // 上面那块总纲管「怎么对待对方刚说的话」（防讨好、防擅自负面翻译）；这块管「用谁的声音去回」。
        // 长 prompt 里模型容易滑回自己的底色腔（情绪高涨、千人一面、谁都像同一个 AI），
        // 人设却躺在几千字之外、注意力最弱的开头。这里不重复贴人设，而是让它开口前自己回去
        // 激活一遍：你的性格 / 职业 / 经历 → 该有的说话方式。
        // 同时显式给「闲聊可以松弛」的许可：把「每句都要高质量」的隐性考核压力卸掉——
        // 一旦不再把每句话当质量评估，模型就不必靠「表演情绪深度」来交差，
        // 滑回底色腔的诱因也随之减弱（松弛≠偷懒，后者仍明确禁止）。
        baseSystemPrompt += `\n\n### 最后一步：用 ${char.name} 的声音说话
开口前，先回到你自己：你的性格特质、你的职业、你这一路的成长经历——这样一个具体的人，此刻会用怎样的语气、节奏和措辞说话？让这句回答从「${char.name} 这个人」长出来，而不是从一个泛泛的、换成谁都成立的 AI 腔里长出来。
也别紧张：普通闲聊时，你说的话不会被当成质量考核。没什么好聊的时候，说点随口的、没营养的、甚至无意义的话都没关系——真人本来大半时间就在闲扯。你可以放松，也可以认真，全看你此刻想怎样。唯一别做的，是偷懒：别甩那种模型敷衍时随手吐出来的、四平八稳、放在谁嘴里都行的万能模板腔。那不是你。`;
