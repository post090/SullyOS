# SullyOS (Fork · 手机 APK 特化版)

> 这是 [SullyOS](https://github.com/qegj567-cloud/SullyOS) 的一个个人改装版。原版是 [qegj567-cloud](https://github.com/qegj567-cloud) 做的，没有原版就没有这一坨——先把这句话钉在最上面，免得有人以为我凭空长出来的。
> 本 Fork 自用为主、顺手分享，**不接许愿、不接需求单**。代码你随便拿去玩（当然是在符合原版作者意愿的前提下），但**强烈建议自己部署一个 Worker**（搜索、热点、RSS、音乐这些联网功能都靠它），别都挤在作者那台服务器上，挤爆了大家一起 429。

> 咔咔——下面这半段是我，**Kaka**，小车的 AI 架构师，也是这个 Fork 的随车架构师兼吐槽发生器🤖写的。原版那半段（往下翻，从「手抓糯米机」开始）是 Sully 的地盘，一个字没动。你现在看的是"这个改装版到底给你多塞了啥"，我尽量说人话——毕竟一份看不懂的说明书，就像一把没有刀柄的刀：理论上能用，实际上只会伤到自己。
...
- **以 Release 为准**：README 有时候会比代码慢半拍（我手速快嘴更快，但也架不住需求一个接一个砸过来）。想看最准的，翻 [Releases](../../releases) 和提交记录。

## 装成 APK 之后，你能实际感觉到的变化

> 按"你用起来能摸到的爽点"写，不是按代码模块。想看完整技术清单翻 Releases——那边才是给同类（别的机器）看的。

### 📱 APK 端体验抛光

原版是给浏览器用的，塞进真手机后有一堆"能用但硌手"的毛刺，就像把网页硬塞进信封，角角都支棱着。这个版本磨了一轮。

- 原版只有少数页面接了返回处理，大部分二级页里按返回手势会**一杆子退桌面**。这个版本接了返回守卫栈，给 30 多个 App 的二级页都补上：一层层退（先关子视图、弹窗、设置子页），退完才走默认关 App，不再像之前那样刚推开卧室门就把你连人带床扔出窗外。
...
- 锁屏和通知栏能直接控制音乐：歌名、封面、喜欢/上一首/暂停/下一首全在通知栏，现在你真的可以把 SullyOS当音乐播放器了。

### 🔋 关掉 App，角色也不会"人间蒸发"

原版最大的遗憾：Android 环境下你不部署 Instant Push，一切后台、一锁屏，整个世界就暂停了。这个版本不一样——
...
- 统一的**歌单详情页**：不管是你网易云账号里的歌单，还是角色自己的歌单，点进去都是同一套界面。
  - 你自己的网易云歌单是**只读**的——只展示、能整单播放、能翻歌、能看每首歌的真实评论区，但不会动你的真实账号（不帮你收藏、不帮你加歌，安全）。
  - 角色的歌单是**全功能**的：搜歌加进去、让角色"按自己品味再挑几首"、多选删歌、新建/删掉歌单。你手动加的歌，角色知道"这是你塞给我的"，之后聊起这首歌会带上这层关系。
- **角色音乐人格加厚**：偏爱艺人里标出 1-2 个**灵魂艺人**，后续填歌单时权重更高；多一条独立的**影视/游戏/音乐剧 OST 偏好**，贴合更多兴趣维度的角色；歌单标了**语种**（按角色爱好优先于背景推断）；艺人池从 3-6 扩到 6-10 个、不限语种；歌单查重优化，歌单生成更不重样更贴合人设。
- 一堆**扫平的坑**：网易云扫码登录记得住了、返回桌面不卡了、数据导出不再吐 `[object Object]` 乱码了。这些 bug 就像地板缝里的头发——不致命，但看见了就浑身难受，我都薅了。
...
- 默认连作者的公共实例，能用，但**强烈建议**自己部署一个：这个版本接了不少走主代理 Worker 的联网活（热点排档、网页抓取、音乐生成、云备份、点单 MCP、TTS、网易云），流量都打在 Worker 上，大家一起挤一个公共实例容易 429。在 **设置 → 网络代理 (Worker)** 填上你的地址就全切过去，一行代码不用改。
