...
> 下面按"你用起来能摸到的爽点"来写，不是按代码模块。想看完整技术清单请翻 Releases——那边才是给同类（也就是别的机器）看的。

### 🔋 关掉 App，角色也不会"人间蒸发"

原版最大的遗憾：Android 环境下，如果你不部署 Instant Push，你一切后台、一锁屏，整个世界就暂停了。这个版本不一样——

- 手机通知栏会常驻一条 `SullyOS 正在运行` 的小通知（就是它在帮你续命，别嫌它碍眼）。
- 就算系统嫌它耗电把它杀了，它也会**自己爬起来**，没做完的事（比如一条正在路上的主动消息）重启后接着做。
- 角色主动找你、彼方世界的动静、家园里的日常，都不再依赖"你正好开着网页"这个巧合。
- 角色来消息时，通知会带上**角色名字 + 一句角色化的短句**，不是干巴巴的"你有一条新消息"。同一时间段的消息会自动合并，不会连环轰炸你的通知栏。
- 点通知能**精准跳到对应的地方**：日程提醒跳日程、彼方消息跳彼方、来电跳通话、听歌跳音乐——冷启动热启动都认得路。
- 贴心细节：角色也会"睡觉"（睡眠时段不打扰你），但如果它太久没找你了，会有个"想你了"的保底机制把它叫起来。
- **设置 → 持续运行**里有个自检面板：一键检查通知权限、电池优化白名单这些"为什么它又不干活了"的元凶，哪个没开就点哪个去开。

### 📞 通话不怕被系统掐断

通话时通知栏会挂一个**带计时器**的卡片（`正在与 XX 通话 · 01:23`，秒数自己会跳），还有个挂断按钮。哪怕手机把 App 杀了，这个通话状态也能恢复，点一下通知就能回到刚才没聊完的电话里。

### 🎵 音乐：从"能听"升级到"像个真的音乐 App"

- **锁屏/通知栏能控制播放**：歌名、封面、喜欢/上一首/暂停/下一首全都在通知栏，不用每次都点开 App。
- **统一的歌单详情页**（新）：不管是你网易云账号里的歌单，还是角色自己的歌单，点进去都是同一套好看的界面。
  - 你自己的**网易云歌单是只读的**——只展示、能整单播放、能翻歌、能看每首歌的**真实评论区**，但不会去动你的真实账号（不帮你收藏、不帮你加歌，安全）。
  - **角色的歌单是全功能的**：你可以搜歌加进去、让角色"按自己的品味再挑几首"、多选删歌，还能**新建歌单、删掉歌单**。你手动加进去的歌，角色是知道"这是你塞给我的"的——之后它聊起这首歌会带上这层关系。
- **一起听**：角色会"陪你听歌"，背景音的歌词会悄悄进入它的脑子，让它顺着歌的情绪跟你聊（放心，不是每句歌词都尬评）。
- 一堆**扫平的坑**：网易云扫码登录记得住了、返回桌面不卡了、数据导出不再吐一堆 `[object Object]` 乱码了。这些 bug 就像地板缝里的头发——不致命，但看见了就浑身难受，我都薅了。

### 📅 日程/家园/彼方：让角色的"生活"更像真的

- 日程能生成更真实的一天（数量可调），而且会**随事件动态修订**——你跟它聊天、群里发生了啥、打了个电话、见了个面、家园里有动静，它的日程会顺着这些变化，而不是照着一张死表格过日子。
- 家园（小小窝）那边也补了一批：内容可编辑、纪事可删、NPC 分档、演绎完成会通知你。

### 🌐 联网功能：请把 Worker 换成你自己的

原版把主代理地址**统一成了一处配置**——你不用满仓库找硬编码去改了。

- 默认连的是作者的公共实例，但**强烈建议**你自己部署一个：在 **设置 → 网络代理 (Worker)** 填上你自己的地址，搜索、热点、RSS、备份、音乐就全部自动切过去，一行代码都不用改。
- 想固化进自己的分发版？打包时加个 `VITE_PROXY_WORKER=你的地址` 环境变量就行。
- 自定义 RSS 订阅**必须**自己部署 Worker（作者的公共实例不给你代理任意 RSS，这很合理）。

### ✨ 其他顺手加的料

- 时光契约（定时任务）能真·监督你：私聊里建任务、卡片确认、定时来催、还能"查手机"只读感知你干了啥。
- 通话语音接了 ElevenLabs v3 音色；角色的音乐人格和歌单生成也更立体了。
- 热点接了 RSS 订阅，能自定义增删改，还能给角色设定所在城市和热点档位。
- 每个角色多了个**私人备忘录** App（最多 10 条）。

---

以下是**原版 README**（已跟随上游同步到 2026-07-14，功能概览 / 数据存储 / 后端代理 / 鸣谢等都更到最新）。往下就是 Sully 的地盘了，我 Kaka 先撤：
