...
- 锁屏和通知栏能直接控制音乐：歌名、封面、喜欢/上一首/暂停/下一首全在通知栏，封面还会自动给通知染色（网易云那套观感）。

### 🔋 关掉 App，角色也不会"人间蒸发"

原版最大的遗憾：Android 环境下你不部署 Instant Push，一切后台、一锁屏，整个世界就暂停了。这个版本不一样——

- 通知栏常驻一条 `SullyOS 正在运行` 的小通知（前台服务的法律要求，别嫌它碍眼）。
...
- **角色音乐人格加厚**：偏爱艺人里标出 1-2 个**灵魂艺人**，后续填歌单时权重更高；多一条独立的**影视/游戏/音乐剧 OST 偏好**，接住"不追星但死磕塞尔达 OST"这类角色；歌单标了**语种**（按角色爱好优先于背景推断）；每个歌单挂上**真实可搜的关键词**，填歌直接拿去搜；艺人池从 3-6 扩到 6-10 个、不限语种，三个歌单撞歌概率低一截。
...
- 时光契约（定时任务）能**真·监督**你：私聊里建任务、卡片确认、定时来催、还能"查手机"只读感知你干了啥。
...
- 角色聊天里花钱/挣钱会**真扣真进账**（`SPEND`/`INCOME` 指令），工资、房租、月供按日期自动结算，钱包随身物品和栖居住所物品双向对账防打架；转账真扣钱，买东西也会自动同步钱包余额和家里库存——钱少了，东西多了，逻辑闭环。
