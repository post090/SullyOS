...
- 支持桌面图标自由拖动和更换位置，还优化了从 App 返回桌面时的视觉表现。
...
- 默认连作者的公共实例，能用，但**强烈建议**自己部署一个：这个版本接了不少走主代理 Worker 的联网活，流量都打在 Worker 上，大家一起挤一个公共实例容易 429。在 **设置 → 网络代理 (Worker)** 填上你的地址就全切过去，一行代码不用改。
