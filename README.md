...
> NuiAPK（以下简称 Nui）是 [SullyOS](https://github.com/qegj567-cloud/SullyOS) 的一个自用 Fork。**本 Fork 面向 APK 做了整体优化，以及增加了一些功能和新 APP。** 原版是 [qegj567-cloud](https://github.com/qegj567-cloud) 做的，没有原版就没有这一坨——先把这句话钉在最上面，免得有人以为我凭空长出来的。

> Nui 自用为主、顺手分享，**不接许愿、不接需求单**。代码你随便拿去玩 **（当然是在符合原版作者意愿的前提下）**，但**强烈建议自己部署一个 Worker**（搜索、热点、RSS、音乐这些联网功能都靠它），别都挤在作者那台服务器上，挤爆了大家一起 429。

> ⚠️ 本 Fork 的大量修改是冲着 APK 去的，**网页端 / 其它端没测过，也没有适配计划**。要在其它端用，建议你自己过一遍检查。

> 咔咔——下面这半段是我，**Kaka**，小车的 AI 架构师，也是 Nui 的随车架构师兼吐槽发生器🤖写的。原版那半段（往下翻，从「手抓糯米机」开始）是 Sully 的地盘，一个字没动。你现在看的是"Nui 到底给你多塞了啥"，我尽量说人话——毕竟一份看不懂的说明书，就像一把没有刀柄的刀：理论上能用，实际上只会伤到自己。

## Nui 到底改了啥？

一句话：**原版是装在浏览器里的虚拟手机，我把它塞进了真正的安卓手机，并且让它学会了"你不看它的时候也不装死"。**

- **跟着原版走**：定期同步上游更新，原版有的我都留着，撞车时优先保原版体验。我只做加法，不拆原版的台。
- **主打一个"活着"**：原版角色只在你打开网页时才醒着；Nui 打包成 APK 后，能在后台悄悄活着——该提醒的提醒、该主动找你的主动找你，不会你一锁屏它就集体昏迷。
...
> 按"你用起来能摸到的爽点"写，不是按代码模块。

### 📱 APK 端体验抛光

原版是给浏览器用的，塞进真手机后有一堆"能用但硌手"的毛刺，就像把网页硬塞进信封，角角都支棱着。Nui 磨了一轮。

- 原版只有少数页面接了返回处理，大部分二级页里按返回手势会**一杆子退桌面**。Nui 接了返回守卫栈，给 30 多个 App 的二级页都补上：一层层退（先关子视图、弹窗、设置子页），退完才走默认关 App，不再像之前那样刚推开卧室门就把你连人带床扔出窗外。
- 样式引擎以前运行时从 `cdn.tailwindcss.com` 现场下载，墙一挡整个 App 就**裸奔成无样式文档流**（欢迎页、锁屏、桌面全坏，比光着身子出门还惨）。现在打包进 APK 自带，本地加载失败才回退 CDN。
...
原版最大的遗憾：Android 环境下你不部署 Instant Push，一切后台、一锁屏，整个世界就暂停了。Nui 不一样——

- 通知栏常驻一条 `SullyOS 正在运行` 的小通知（别嫌它碍眼，APK保命常用手段）。
- 系统嫌它耗电把它杀了，它也会**自己爬起来**——靠 `START_STICKY` + 开机自启 + 精确闹钟三件套，没做完的事重启后接着做。
- 角色主动找你、彼方世界的动静、家园日常，都不再依赖"你正好开着网页"这个巧合。
- 角色来消息时通知带角色名字 + 一句角色化的短句，不是干巴巴的"你有一条新消息"；同一时段的消息自动合并，不会连环轰炸。
...
通话状态会持久化到原生层——哪怕手机把 App 杀了，重新点通知能回到刚才没聊完的电话里，通话记录和时长都还在。（计时卡片见上面的「APK 端体验打磨」。）

### 🎵 音乐：从"能听"升级到"像个真的音乐 App"

- **统一歌单详情页**：网易云歌单和角色歌单点进去是同一套界面。网易云只读（整单播放、看真实评论区，不动你真账号）；角色歌单全功能（搜歌加、按品味再挑、多选删、新建删）。你手动加的歌，角色知道"这是你塞给我的"。
- **角色音乐人格加厚**：偏爱艺人标灵魂艺人、独立 OST 偏好、歌单标语种、艺人池扩到 6-10 个——歌单更贴人设、更不重样。
- **播放体验补齐**：长按歌词多选分享、歌手页页签切换、歌曲操作菜单全站统一（下一首/收藏/评论/歌手/专辑/分享）。艺人头像和 OST 封面本地缓存，跨角色共享秒开。
- **分享给角色**：歌单/搜索/专辑里任意一首都能丢进角色聊天窗；还能长按歌词自选几句分享，带附言，角色知道你分享的是哪段。

### 📅 日程/家园/彼方：让角色的"生活"更像真的

- 日程能生成更真实的一天（数量可调），而且会**随事件动态修订**——你跟它聊天、群里发生了啥、打了个电话、见了个面、家园有动静，它的日程会顺着变化，而不是照着一张死表格过日子。
- 家园（小小窝）那边也补了一批：内容可编辑、纪事可删、NPC 分档、演绎完成会通知你。

### 🌐 联网功能：请把 Worker 换成你自己的

原版把主代理地址**统一成一处配置**，你不用满仓库找硬编码改了。

- 默认连作者的公共实例，能用，但**强烈建议**自己部署一个：Nui 接了不少走主代理 Worker 的联网活，流量都打在 Worker 上，大家一起挤一个公共实例容易 429。在 **设置 → 网络代理 (Worker)** 填上你的地址就全切过去，一行代码不用改。
- 想固化进自己的分发版？打包时加个 `VITE_PROXY_WORKER=你的地址` 环境变量就行。

### ✨ 其他顺手加的料

- 时光契约能**真·监督**你：私聊里建任务、卡片确认、定时来催、还能"查手机"只读感知你干了啥。
- 通话语音接了 ElevenLabs v3 音色。
- 热点接了 RSS 订阅，能自定义增删改，还能给角色设定所在城市和热点档位。
...
前面说过联网要走你自己的 Worker，这里讲怎么落地。本地数据（角色、记忆、备份）都在你设备上，不碰服务器。按「必须 → 可选 → 不用管」来：

### ① 必须 · 主代理 Worker（联网总开关）

一行命令部署 `worker/index.js`（搜索、热点、云备份、Notion、飞书、点单 MCP、网页抓取、TTS、音乐生成、网易云、小红书 Lite 全在这一个文件里）：

```bash
npx wrangler deploy worker/index.js --name <你的Worker名> --compatibility-date 2024-01-01
```

部署完到 App「**设置 → 网络代理 (Worker)**」填地址，联网能力一键全切到你名下。想固化进分发版：

```bash
VITE_PROXY_WORKER=https://你的地址.workers.dev npm run build
```

> 网易云默认走 Worker 里 `NETEASE_UPSTREAMS`（作者部署的 api-enhanced），不换也能用；想自建就把自己的 Vercel 地址填进去重新部署。
