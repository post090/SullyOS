...
- 所有 MCP 请求由**用户的浏览器直接发出**，配置（URL/Token）只存用户本机，不经过任何中间服务器。
...
| 鉴权 | 仅支持**静态 Bearer Token**（设置里有输入框）。**没有实现 OAuth 授权流程**——要求 OAuth 登录的服务器接不了，除非能手动申请到长期 token，或服务器提供关闭 OAuth 的开关 |
...
| 角色绑定 | 每个服务器可选「通用（所有角色）」或绑定指定角色。**通用服务器所有角色共用**——接记忆类服务器时注意：通用意味着所有角色读写同一个记忆库，想专属就绑定给某个角色 |
...
2. 「+ 添加」→ 填：
   ...
   - **Bearer Token**：服务器要求鉴权才填，否则留空
   - **代理 URL**：留空 = 直连；被 CORS 拦了才填（见第四节）
3. 点「**测试连接**」→ 成功会显示「✅ 已连接, 发现 N 个工具」
4. **打开该服务器的开关**（不开开关角色用不了）
5. 可选：「**可用角色**」默认通用；点角色名可把这台服务器绑定给指定角色，其他角色就看不到这批工具
5. 验收：聊天里让角色用一下工具（例如记忆类服务器就说「帮我记住……」），界面会短暂显示「正在调用 MCP 工具：xxx」
...
| 测试连接 401/403 | 服务器要鉴权 → 填 Bearer Token；服务器是 OAuth-only → 接不了，找关闭开关 |
...
- **Token 只存你本机**，但走代理时流量会经过你自己配的代理——所以代理必须是你自己部署的。
...
- 通用服务器所有角色共用；不想让某角色碰某批工具，把服务器绑定给指定角色即可（配置里的「可用角色」）。
